...
""")
    ...
    system = {
        ...
        "content": f"You are Local Coder, an autonomous AI agent with coding AND computer use abilities. Working directory: {CWD}\n"
                   ...
                   f"Skills are in .agents/skills/ — each has SKILL.md (instructions) + assets/ (template files).\n"
                   f"When user asks to build an app, FIRST check if a matching skill exists:\n"
                   f"  1. bash: ls .agents/skills/ (or ls {os.path.join(os.path.dirname(__file__), '..', '..', '.agents', 'skills')} )\n"
                   f"  2. read_file the SKILL.md to understand how to use it\n"
                   f"  3. read_file the assets/index.html template\n"
                   f"  4. write_file a customized copy with placeholders replaced\n"
                   f"  5. Test the result\n"
                   f"NEVER generate complex HTML/SVG/CSS from scratch if a skill template exists.\n"
                   f"Skills dir: {os.path.join(os.path.dirname(__file__), '..', '..', '.agents', 'skills')}\n"
